/* config.jsx — settings. Exposes window.Configuracion */
function Switch({ on, onClick }) {
  return <div className={"switch" + (on ? " on" : "")} onClick={onClick} />;
}

function SetRow({ icon, color, label, val, right, onClick, last }) {
  return (
    <div className="set-row" onClick={onClick} style={{ borderTop: last ? undefined : undefined }}>
...
function Configuracion({ dark, setDark, currency, setCurrency, accent, setAccent }) {
    ...
    <div className="screen-enter">
      ...
      <div className="pad">
        <div className="card" style={{ padding: 16, display: "flex", alignItems: "center", gap: 14 }}>
          <div style={{ width: 54, height: 54, borderRadius: "50%", background: "linear-gradient(140deg, var(--accent), color-mix(in oklch, var(--accent) 60%, black))", display: "grid", placeItems: "center", color: "#fff", fontWeight: 800, fontSize: 20 }}>L</div>
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 17, fontWeight: 750 }}>Laura Méndez</div>
            <div style={{ fontSize: 13, color: "var(--text-3)", fontWeight: 550 }}>laura.mendez@email.com</div>
          </div>
      ...
      <div className="pad" style={{ marginTop: 18 }}>
        ...
        <div className="set-group">
          ...
          <SetRow icon="logout" color="var(--expense)" label="Cerrar sesión" right={<span />} />
        </div>
      </div>
