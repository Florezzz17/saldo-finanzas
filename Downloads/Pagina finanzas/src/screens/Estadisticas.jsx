...
import { CatIcon, Donut, Bars } from '../components/ui';
...
export function Estadisticas() {
  ...
  const source = mode === "expense" ? month.byCat : (() => {
    const o = {};
    txs.filter(t => t.type === "income" && isThisMonth(t.date)).forEach(t => { o[t.cat] = (o[t.cat] || 0) + t.amount; });
    return o;
  })();

  const segs      = Object.entries(source).map(([id, val]) => ({ cat: catMap[id], val })).filter(x => x.cat).sort((a, b) => b.val - a.val);
